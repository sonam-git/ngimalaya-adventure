...
const TrekMapModal: React.FC<TrekMapModalProps> = ({ isOpen, onClose, trek }) => {
  ...
  useEffect(() => {
    ...
    const loadMapData = async () => {
      ...
      try {
        // Transform trek itinerary to the format needed for geocoding
        // Use location field if available, otherwise try to extract from title
        const trekDays = trek.itinerary.map((day) => ({
          day: day.day,
          title: day.title,
          location: day.location || extractMainLocation(day.title) || `${trek.region}, Nepal`,
        }));

        const coordinates = await processTrekItinerary(trekDays);
  ...
  // Helper function to extract main location from trek day title
  const extractMainLocation = (title: string): string | null => {
    // Remove common prefixes
    const cleanTitle = title.replace(/^(Trek|Fly|Drive|Rest day at|Acclimatization at|Explore|Day off at)\s+/i, '');
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              >
                Interactive map showing the complete trekking route
              </p>
              {/* Important Notice */}
              <div
            ...
            <>
              ...
              >
                ...
                <div
                  ...
                  style={{ fontFamily: 'Lato, "Open Sans", Roboto, sans-serif', fontWeight: 400 }}
                >
                  🔄 <strong>Revisited Locations:</strong> When the trek returns to the same location on different days, 
                  markers are spread in a circular pattern with different blue shades for easy identification. 
              ...
              >
                <p style={{ fontFamily: 'Lato, "Open Sans", Roboto, sans-serif', fontWeight: 400 }}>
                  ...
                  The route line connects all trek locations in order. Zoom and pan to explore the full route.
                </p>
              </div>
            </>
          )}
